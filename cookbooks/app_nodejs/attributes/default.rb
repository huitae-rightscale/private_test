...
default[:app_nodejs][:bin_baseurl]  = "http://nodejs.org/dist/v#{node[:nodejs][:version]}"
default[:app_nodejs][:tar]          = "node-v#{node[:nodejs][:version]}-linux-x64.tar.gz"
default[:app_nodejs][:source]       = "http://nodejs.org/dist/v#{node[:nodejs][:version]}/#{node[:nodejs][:tar]}"
